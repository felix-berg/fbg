...
*/
void draw_stroke_part_vertical(Frame & frame, int x, int y, Rgba color, int sw) 
{
   int lOff, rOff;
...
void draw_stroke_part_horisontal(Frame & frame, int x, int y, Rgba color, int sw)
{
   int lOff, rOff;
   offsets_from_strokeweight(sw, &lOff, &rOff);
   int fx = x - lOff; 
...
void fbg::compute_AA_rect_stroke(Frame & frame, int rx, int ry, int rw, int rh, Rgba color, int sw) {
...
void fbg::compute_AA_rect_fill(Frame & f, int rx, int ry, int rw, int rh, Rgba color) 
{
   // bottom right point
   const int brx = rx + rw;
   const int bry = ry + rh;
